...
function handleDashboardPage(req, res, db, url) {
  ...
  const locationActive = locationPrefs.metros.length > 0 || locationPrefs.remote === false || locationPrefs.includeUnknown === false;
  ...
  // to the user's chosen metro. Otherwise fall through to the SQL stats.
  let overrideStats = null;
  if (locationActive) {
    const allRows = db.prepare("SELECT status, stage, location FROM jobs").all();
    const filteredRows = allRows.filter((j) => passesPrefs(j.location, locationPrefs, metros));
  ...
  const baseJobs = isPaginatedListView && locationActive
    ? rawBase.filter((j) => passesPrefs(j.location, locationPrefs, metros))
    : rawBase;
