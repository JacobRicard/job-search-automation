...
const { GROQ_DAILY_LIMIT, DAILY_TARGET } = require('../config/constants');
...
function handleScoringProgress(req, res, db) {
  ...
  const delayMs = process.env.OLLAMA_HOST ? 30_000 : (parseInt(process.env.GROQ_RATE_DELAY_MS, 10) || 500);
...
function fetchDashboardContext(db, overrideStats) {
  ...
  const apiUsage = { used: usageRow ? usageRow.used : 0, limit: GROQ_DAILY_LIMIT };
