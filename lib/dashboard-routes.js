...
const { logEvent, touchJob, getJobById, getGlobalStats, getAppliedByCompany } = require('./db');
...
const { readApplicationOverrides } = require('./application-overrides');
...
function buildApplicationPrepPayload(job, prep) {
  const overrides = readApplicationOverrides(job.id);
  const questions = prep?.questions?.length ? prep.questions : (overrides.questions || []);
  const answers = (prep?.answers && Object.keys(prep.answers).length) ? prep.answers : (overrides.answers || {});
  return {
    jobId: job.id,
    title: job.title,
    company: job.company,
...
function fetchFilteredJobs(db, filter, sort, level, options = {}) {
  const includeTailoredResumeStatus = options.includeTailoredResumeStatus !== false;
  ...
  if (includeTailoredResumeStatus) attachTailoredResumeStatus(db, jobs);
...
function attachTailoredResumeStatus(db, jobs) {
  ...
  const rows = db.prepare(`
    SELECT job_id, status, resume_pdf_path
...
function computeMyLevelCount(db, filter, sort, level, searchOptions, baseJobs, allJobs, isPaginatedListView) {
  const baseJobsForLevel = level === '1'
    ? baseJobs
    : fetchFilteredJobs(db, filter, sort, null, { includeTailoredResumeStatus: false });
...
function handleDashboardPage(req, res, db, url) {
  ...
  const baseJobs = fetchFilteredJobs(db, filter, sort, level, { includeTailoredResumeStatus: false });
  ...
  if (isPaginatedListView) attachTailoredResumeStatus(db, jobs);
  ...
  const html = renderDashboard({ jobs, pagination, filter, sort, level, searchOptions, ...context, analyticsData, marketResearchData, slugHealth: loadSlugHealth(), jdHealth: loadJdHealth() });
