...
const { DAILY_TARGET } = require('../config/constants');
...
const { computeApplicantYoe } = require('./seniority');
const { renderHelpPage } = require('./html/help-page');
const { jsonOk, jsonError, route, postRoute } = require('./routes/_helpers');
const { parseDashboardSearchOptions, normalizeDashboardPage, applyDashboardSearch } = require('./dashboard-search');
...
function handleScoringProgress(req, res, db) {
  const row = db.prepare(`
    SELECT
      SUM(CASE WHEN score IS NOT NULL THEN 1 ELSE 0 END) AS scored,
      SUM(CASE WHEN score IS NULL AND status = 'pending' THEN 1 ELSE 0 END) AS unscored,
      MAX(CASE WHEN score IS NOT NULL THEN updated_at END) AS latestScoreAt
    FROM jobs
  `).get();
  const scored = row.scored || 0;
  const unscored = row.unscored || 0;
  const total = scored + unscored;
  const latestScoreAt = row.latestScoreAt || null;
  let active = false;
  if (latestScoreAt) {
    const lastMs = Date.parse(latestScoreAt + 'Z');
    if (Number.isFinite(lastMs)) active = (Date.now() - lastMs) < 120_000;
  }
  // Ollama: ~30s per job on a consumer GPU. Groq: rate-delay based.
  const delayMs = process.env.OLLAMA_HOST ? 30_000 : 2_000;
  const etaSeconds = unscored > 0 ? Math.ceil((unscored * delayMs) / 1000) : 0;
  const todayStr = new Date().toLocaleDateString('en-CA');
  const usageRow = db.prepare(
    "SELECT COALESCE(SUM(call_count), 0) AS used FROM api_usage WHERE date = ?"
  ).get(todayStr);
  const apiUsed = usageRow ? usageRow.used : 0;
  // Surface quota-exhausted state — once any job has a "quota exceeded"
  // error stored, the daily limit has been hit and remaining unscored
  // jobs won't be picked up until the quota resets.
  const quotaErrRow = db.prepare(`
    SELECT 1 FROM jobs
    WHERE score IS NULL
      AND score_error IS NOT NULL
      AND (
        score_error LIKE '%exceeded%quota%'
        OR score_error LIKE '%RESOURCE_EXHAUSTED%'
        OR score_error LIKE '%rate_limit_exceeded%'
        OR score_error LIKE '%rate limit%'
      )
    LIMIT 1
  `).get();
  const quotaExhausted = !process.env.OLLAMA_HOST && !!quotaErrRow;

  // Read pipeline phase written by refresh.js so dashboard can show "Scraping..." vs "Scoring..."
  let phase = 'idle';
  let phaseStep = '';
  try {
    const { baseDir } = require('../config/paths');
    const status = JSON.parse(fs.readFileSync(path.join(baseDir, 'pipeline-status.json'), 'utf8'));
    phase = status.phase || 'idle';
    phaseStep = status.step || '';
  } catch {}

  jsonOk(res, { active, scored, unscored, total, etaSeconds, latestScoreAt, apiUsed, quotaExhausted, phase, phaseStep });
}
...
function handleResume(req, res, db, url) {
  const profileDir = require('../config/paths').baseDir;
  const resumePath = path.join(profileDir, 'resume.pdf');
  ...
  res.writeHead(404); res.end('Resume PDF not found. Run: node generate-resume.js');
}
...
function fetchFilteredJobs(db, filter, sort) {
...
function fetchDashboardContext(db, overrideStats) {
  ...
  const apiUsage = { used: usageRow ? usageRow.used : 0, limit: null };
...
const MARKET_RESEARCH_PROFILE_DIR = require('../config/paths').baseDir;
const MARKET_RESEARCH_CACHE_PATH = path.join(MARKET_RESEARCH_PROFILE_DIR, 'market-research-cache.json');
const RESUME_PATH_FOR_RESEARCH = path.join(MARKET_RESEARCH_PROFILE_DIR, 'resume.md');
...
function handleDashboardPage(req, res, db, url) {
  ...
  const requestedPage = normalizeDashboardPage(url.searchParams.get('page'));
  ...
  const rawBase = fetchFilteredJobs(db, filter, sort);
  ...
  const isFirstRun = db.prepare("SELECT COUNT(*) as n FROM jobs").get().n === 0;

  const html = renderDashboard({
    jobs, pagination, filter, sort, searchOptions,
    ...context,
...
const handleToggleResume = postRoute(async ({ id, value }, res, db) => {
  db.prepare('UPDATE jobs SET resume_requested = ? WHERE id = ?').run(value, id);
  jsonOk(res, { ok: true });
});

const handleResumeExport = route((req, res, db) => {
  const jobs = db.prepare('SELECT title, company, url, description FROM jobs WHERE resume_requested = 1').all();
  const resumePath = path.join(MARKET_RESEARCH_PROFILE_DIR, 'resume.md');
  let resume = '';
  try { resume = fs.readFileSync(resumePath, 'utf8'); }
  catch { return jsonError(res, 404, 'resume.md not found — add it to data/'); }
  jsonOk(res, { resume, jobs });
});

const handleDeleteUnselected = route((req, res, db) => {
  db.prepare('DELETE FROM jobs WHERE resume_requested = 1').run();
  jsonOk(res, { ok: true });
});
