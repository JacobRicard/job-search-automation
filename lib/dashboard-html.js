...
const { renderAnalytics, renderActivityLog } = require('./html/analytics');
...
function renderDashboard({ jobs, pagination, filter, sort, searchOptions, dailyDigest, dailyCounts, globalStats, appliedByCompany, apiUsage, scraperHealth, companyTags, analyticsData, marketResearchData, slugHealth, jdHealth, locationPrefs, metros, locationHiddenCount, isFirstRun }) {
  ...
  if (filter === 'analytics') {
    ...
    bodyHtml = renderMarketResearch(
      marketResearchData?.cache,
      marketResearchData?.jobCount || 0,
      marketResearchData?.allJobs,
      marketResearchData?.applicantYoe,
      marketResearchData?.appliedCount || 0,
      {
        current: marketResearchData?.current,
        allTime: marketResearchData?.allTime,
      }
    );
  } else {
    bodyHtml = renderJobTable(jobs, appliedByCompany, companyTags, filter, sort, pagination, searchOptions);
  }

  const apiIndicator = apiUsage
    ? `<div id="api-indicator" class="api-indicator" data-limit="${apiUsage.limit}" style="font-size:11px;color:${apiUsage.used > apiUsage.limit * 0.8 ? COLORS.red : COLORS.muted};margin-left:8px;opacity:0.8">API: <span id="api-indicator-used">${apiUsage.used}</span>/${apiUsage.limit}</div>`
    : '';
...
<link rel="stylesheet" href="/public/dashboard.css?v=18">
<script>
...
<header>
  <div class="header-bar">
    ...
    <nav class="seg-control">
      ${[
        ...
        return `<button class="seg-opt${active ? ' active' : ''}" onclick="location='${buildDashboardHref(id, sort, searchOptions)}'">
          ${label}<span class="seg-count">${count}</span>
        </button>`;
      }).join('')}
    </nav>
    <div class="header-right">
      <button class="insights-btn" id="refresh-now-btn" title="Run pipeline now" onclick="triggerRefresh(this)">
      ...
      <a href="/settings" class="insights-btn" title="Settings" style="display:inline-flex;align-items:center;justify-content:center;text-decoration:none">
  ...
  ${renderFilters(filter, sort, globalStats, searchOptions, { locationPrefs, metros })}
</header>
...
<div class="page-wrap">
<div id="scoring-progress-banner" style="display:none;background:#1f2937;border:1px solid #374151;padding:10px 14px;margin:8px 16px;border-radius:8px;font-size:13px;color:#e5e7eb;align-items:center;gap:12px">
  <span style="font-size:16px">&#9203;</span>
  <div style="flex:1;min-width:0">
    <div id="scoring-progress-msg" style="font-weight:500"></div>
    <div style="height:4px;background:#374151;border-radius:2px;margin-top:6px;overflow:hidden">
      <div id="scoring-progress-bar" style="height:100%;background:#a855f7;width:0%;transition:width 0.5s ease"></div>
    </div>
  </div>
  <button onclick="document.getElementById('scoring-progress-banner').style.display='none'" style="background:transparent;border:none;color:#9ca3af;cursor:pointer;font-size:14px;padding:0 4px">&times;</button>
</div>
${(() => {
  if (!jdHealth) return '';
...
<div class="resume-action-bar" id="resume-action-bar" style="display:none">
  <span class="resume-bar-count" id="resume-bar-count"></span>
  <button class="btn resume-copy-btn" onclick="copyForClaude(this)">Copy for Claude</button>
  <button class="btn resume-delete-btn" onclick="deleteUnselected(this)">Delete</button>
  <span class="resume-bar-msg" id="resume-bar-msg"></span>
</div>
${bodyHtml}
</div>
<div class="toast" id="toast"></div>
${renderModals()}
<script>window.__FIRST_RUN__ = ${isFirstRun ? 'true' : 'false'};</script>
<script src="/public/dashboard.js?v=15"></script>
<script>
(function() {
  var data = ${JSON.stringify(dailyCounts || [])};
  window.initDigestChart = function() {
    if (window._insightChart || !data.length || !document.getElementById('digest-chart')) return;
    window.loadDashboardChart(function() {
      if (window._insightChart) return;
