...
function renderDashboard({ jobs, pagination, filter, sort, level, searchOptions, dailyDigest, dailyCounts, globalStats, appliedByCompany, apiUsage, scraperHealth, companyTags, analyticsData, marketResearchData, slugHealth, jdHealth, locationPrefs, metros, locationHiddenCount }) {
...
<header>
  ...
  ${renderFilters(filter, sort, globalStats, level, searchOptions, { locationPrefs, metros })}
</header>
...
${locationHiddenCount > 0 ? `<div class="location-hidden-banner" style="background:#1e293b;border:1px solid #334155;padding:6px 16px;margin:8px 16px;border-radius:6px;font-size:12px;color:#94a3b8;display:flex;align-items:center;gap:8px">
  <span><strong>${locationHiddenCount}</strong> job${locationHiddenCount === 1 ? '' : 's'} hidden by your location filter.</span>
  <button type="button" onclick="clearLocationPrefs()" style="background:none;border:1px solid #475569;color:#cbd5e1;padding:2px 8px;border-radius:4px;font-size:11px;cursor:pointer">Clear filter</button>
</div>` : ''}
${bodyHtml}
</div>
