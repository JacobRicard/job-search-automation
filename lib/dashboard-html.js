...
function renderDashboard({ jobs, pagination, filter, sort, searchOptions, dailyDigest, dailyCounts, globalStats, appliedByCompany, apiUsage, scraperHealth, companyTags, analyticsData, marketResearchData, slugHealth, jdHealth, locationPrefs, metros, locationHiddenCount }) {
...
${locationHiddenCount > 0 ? `<div class="location-hidden-banner">
  <span><strong>${locationHiddenCount}</strong> job${locationHiddenCount === 1 ? '' : 's'} hidden by your location filter</span>
  <button type="button" class="loc-clear-btn" onclick="clearLocationPrefs()" title="Clear location filter" aria-label="Clear location filter">
