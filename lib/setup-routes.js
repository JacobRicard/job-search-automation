...
function buildContextMd({ titles, stack, salary, location, industry }) {
...
function createSetupHandlers({ profileDir, envPath, log, repoRoot }) {
  ...
  async function handleExtractProfile(req, res) {
    try {
      ...
      const prompt = `You are extracting job search preferences from a resume. Given this resume:\n\n${resumeText}\n\nReturn ONLY a JSON object (no markdown fences, no extra text):\n{\n  "titles": ["title1", "title2"],\n  "stack": ["tech1", "tech2"],\n  "salary": 120000,\n  "industry": "finance"\n}\n\n- titles: 3-5 realistic target job titles based on the candidate's experience level and background\n- stack: key technologies, languages, and tools mentioned in the resume (up to 10 items)\n- salary: realistic base salary floor in USD as an integer based on the candidate's experience level and typical market rates for their role and location (null if unclear)\n- industry: ONE of "finance", "tech", "healthcare", "legal", "marketing", or "other" — the candidate's primary industry based on their work history`;
      let result;
      try {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, titles, stack, salary, industry }));
    } catch (e) {
  ...
  async function handleSetupApiKey(req, res) {
    try {
      ...
      if (key && key.trim()) {
        ...
        updateEnvLine('GROQ_API_KEY', trimmed, envPath);
  ...
  async function handleSetupTestKey(req, res) {
    try {
      ...
      if (result.status === 200 || result.status === 429) {
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, error: 'Key rejected by Groq (invalid or expired)' }));
      } else {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, error: `Groq returned ${result.status}` }));
      }
    } catch (e) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: e.message }));
