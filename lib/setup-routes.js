          ...
          const r = https.request(options, (apiRes) => {
            ...
            apiRes.on('end', () => {
              try {
                ...
                resolve({ ok: !!text, text, status: apiRes.statusCode });
              } catch (e) {
                resolve({ ok: false, text: '', status: apiRes.statusCode });
              }
            });
          });
          r.on('error', () => resolve({ ok: false, text: '', status: 0 }));
          r.setTimeout(30000, () => { r.destroy(); resolve({ ok: false, text: '', status: 0 }); });
          r.write(postData);
          r.end();
        });
        if (!extracted.ok) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: false, error: `Gemini returned ${extracted.status}` }));
          return;
        }
