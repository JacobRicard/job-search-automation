...
function buildContextMd({ titles, salary, location, industry }) {
  ...
  const industryLine = (industry || '').trim() || '(not specified)';
...
function buildCompaniesJs(searchTerms, profileDir) {
  ...
  const toWorkdayLines = (arr) => (arr || [])
    .filter(e => e && typeof e === 'object' && e.sub && e.wd && e.board)
  ...
  const wd  = toWorkdayLines(existing?.WORKDAY_COMPANIES);
...
function createSetupHandlers({ profileDir, envPath, log, repoRoot }) {
  ...
  async function handleSetupStatus(req, res) {
    ...
    res.end(JSON.stringify({ resumeContent, contextContent, companiesContent, hasKey: true }));
  }
  ...
  async function handleSetupResume(req, res) {
    try {
      ...
      if (format === 'pdf') {
        // PDF extraction still uses Gemini (Groq does not accept binary uploads).
        // Falls back to asking the user to paste text if no Gemini key is set.
        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey) {
          setupLog.warn('resume PDF upload skipped: no GEMINI_API_KEY in env');
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: false, error: 'no_gemini_key' }));
          return;
        }
  ...
  async function handleExtractProfile(req, res) {
    try {
      ...
      const { callLLMJson } = require('./claude-llm');
      const prompt = `You are extracting job search preferences from a resume. Given this resume:\n\n${resumeText}\n\nReturn ONLY a JSON object:\n{\n  "titles": ["title1", "title2"],\n  "salary": 120000,\n  "industry": "finance"\n}\n\n- titles: 3-5 realistic target job titles based on the candidate's experience level and background\n- salary: realistic base salary floor in USD as an integer (null if unclear)\n- industry: ONE of "finance", "tech", "healthcare", "legal", "marketing", or "other"`;
      let result;
      try {
        result = await callLLMJson(prompt);
      } catch (e) {
        setupLog.error('extract-profile Claude CLI call failed', { error: e.message });
        result = { titles: [] };
      }
      const titles = Array.isArray(result.titles) ? result.titles.join('\n') : '';
      const salary = typeof result.salary === 'number' && result.salary > 0 ? result.salary : null;
      const ALLOWED_INDUSTRIES = ['finance', 'tech', 'healthcare', 'legal', 'marketing', 'other'];
      const industryRaw = typeof result.industry === 'string' ? result.industry.toLowerCase().trim() : '';
      const industry = ALLOWED_INDUSTRIES.includes(industryRaw) ? industryRaw : '';
      setupLog.info('extract-profile completed', { titleCount: titles.split('\n').filter(Boolean).length, salary, industry });
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, titles, salary, industry }));
    } catch (e) {
  ...
  async function handleSetupTestKey(req, res) {
    // Claude CLI uses subscription auth — no API key to test.
    // Just verify the claude binary is reachable.
    const { spawnSync } = require('child_process');
    const claudeBin = process.env.CLAUDE_CODE_EXECPATH || 'claude';
    const result = spawnSync(claudeBin, ['--version'], { encoding: 'utf8', timeout: 10_000 });
    if (result.error || result.status !== 0) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: `claude CLI not found or not working: ${(result.stderr || result.error?.message || '').slice(0, 200)}` }));
      return;
    }
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, version: (result.stdout || '').trim() }));
  }
