  ...
  async function handleExtractProfile(req, res) {
    try {
      ...
      if (!process.env.GROQ_API_KEY) {
        setupLog.warn('extract-profile skipped: no GROQ_API_KEY');
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true, titles: '', stack: '' }));
        return;
      }
      ...
      let result;
      try {
        result = await callGroqJson(prompt, { maxTokens: 400 });
      } catch (e) {
        setupLog.error('extract-profile Groq call failed', { error: e.message });
        result = { titles: [] };
      }
