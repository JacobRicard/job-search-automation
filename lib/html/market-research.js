...
function renderSeniorityBreakdown(allJobs, applicantYoe, appliedCount = 0, options = {}) {
  const {
    id = 'current',
    hidden = false,
    title = 'Market Seniority Distribution',
    countLabel = 'live open pipeline roles',
    sourceLabel = 'JDs',
    note = 'Closed, rejected, ghosted, and archived jobs are excluded from this market view.',
  } = options;

  if (!allJobs || !allJobs.length) {
    return `
  <div id="market-view-${id}" class="market-view-panel" style="${hidden ? 'display:none' : ''}">
    <div class="analytics-section market-section">
      <h2 class="analytics-title">${escapeHtml(title)}</h2>
  ...
  <div class="analytics-section market-section">
    ...
    <p class="analytics-hint">From ${jdSourceCount} ${sourceLabel} with explicit years. Bright = your range. Dark = reach.</p>
  ...
  <div class="${yearEntries.length > 0 ? 'market-research-grid' : ''}">
    <div class="analytics-section market-section">
      <h2 class="analytics-title">${escapeHtml(title)}</h2>
      <p class="analytics-hint">${total} ${escapeHtml(countLabel)} classified by seniority. ${jdSourceCount} from JD experience requirements, ${titleSourceCount} from title only.</p>
      <p class="analytics-hint" style="margin-top:4px">All-time applied count: <span style="font-family:var(--font-mono)">${appliedCount}</span>. ${escapeHtml(note)}</p>
...
function renderMarketViewTabs(currentCount, allTimeCount) {
  return `
  <div class="analytics-section market-section" style="padding:10px 14px">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
      ...
      <div style="font-size:12px;color:${COLORS.muted}">
        <span style="font-family:var(--font-mono)">${currentCount}</span> current &middot;
        <span style="font-family:var(--font-mono)">${allTimeCount}</span> all-time
      </div>
    </div>
  </div>
  <script>
...
function renderMarketResearch(cache, jobCount, allJobs, applicantYoe, appliedCount = 0, datasets = {}) {
  const trackerHtml = renderTrackerSection();
  const currentJobs = datasets.current?.jobs || allJobs || [];
  const allTimeJobs = datasets.allTime?.jobs || currentJobs;
  const currentCount = currentJobs.length;
  const allTimeCount = datasets.allTime?.jobCount ?? allTimeJobs.length;
  const seniorityHtml = renderMarketViewTabs(currentCount, allTimeCount)
    + renderSeniorityBreakdown(currentJobs, applicantYoe, appliedCount, {
      id: 'current',
      title: 'Current Market Seniority Distribution',
      countLabel: 'live open pipeline roles',
      sourceLabel: 'current JDs',
      note: 'Closed, rejected, ghosted, and archived jobs are excluded from this market view.',
    })
    + renderSeniorityBreakdown(allTimeJobs, applicantYoe, appliedCount, {
      id: 'all-time',
      hidden: true,
      title: 'All-Time Market Seniority Distribution',
      countLabel: 'all-time seen roles',
      sourceLabel: 'all-time JDs',
      note: 'Includes closed, rejected, ghosted, and archived jobs to show the historical market shape.',
    });

  if (!cache) return trackerHtml + seniorityHtml + renderRunPage(null, jobCount);
  const stale = Date.now() - cache.generatedAt > 48 * 3600 * 1000;
  const sampleChanged = cache.jobCount != null && Number(cache.jobCount) !== Number(jobCount);
  if (stale || sampleChanged) {
    return trackerHtml + seniorityHtml + renderRunPage(cache.generatedAt, jobCount, { sampleChanged, previousCount: cache.jobCount });
  }
  return trackerHtml + seniorityHtml + renderResults(cache);
}

function renderRunPage(generatedAt, jobCount, options = {}) {
  const reason = options.sampleChanged
    ? `live sample changed from ${options.previousCount} to ${jobCount} JDs`
    : 'results are stale (older than 48h)';
  const lastRun = generatedAt
    ? `<p style="color:#d8b4fe;font-size:13px;margin-bottom:8px">Last run: ${new Date(generatedAt).toLocaleDateString('en-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})} — ${reason}</p>`
    : '';
...
<div class="analytics-wrap">
  <div class="analytics-section market-section" style="max-width:600px">
    ...
    <p style="color:${COLORS.muted};font-size:14px;line-height:1.7;margin-bottom:16px">
      Analyzes live open job descriptions to surface which skills appear most,
      what gaps exist in your resume, and what the market is actually asking for right now.
    </p>
    ${lastRun}
    <p style="color:${COLORS.muted};font-size:13px;margin-bottom:24px">Will analyze all ~<span style="font-family:var(--font-mono)">${jobCount}</span> live open JDs and run Gemini analysis.</p>
...
<div class="analytics-wrap">
  ...
  <div class="analytics-section market-section">
    ...
    <p class="analytics-hint">Skills appearing most frequently across live open job descriptions in your pipeline</p>
  ...
  <div class="analytics-section market-section">
    <h2 class="analytics-title">Top Skills Across All JDs</h2>
    <p class="analytics-hint">Raw skill frequency across live open JDs for reference</p>
    <div style="max-width:720px;margin-top:8px">${skillRows || '<div style="color:var(--text-muted)">No data</div>'}</div>
  </div>` : ''}
