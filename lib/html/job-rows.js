...
function renderJobTitle(job, eid, eTitle) {
  const url = String(job.url || '').trim();
  if (url) {
    return `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">${eTitle}</a>`;
  }

  return `<button class="job-title-missing-url" type="button" title="No posting URL stored. Show saved job description." onclick="openJobDescription(${jsString(eid)},${jsString(job.title)},${jsString(job.company)})">${eTitle}</button>`;
}
...
function renderJobListHeader(filter, sort, searchOptions) {
  ...
  <button class="sort-col-btn${scoreActive ? ' active' : ''}" onclick="location='${buildDashboardHref(filter, 'score', searchOptions)}'">Score ${scoreActive ? '&#8595;' : '&#8593;'}</button>
  <div class="job-list-header-cell">Title</div>
  <button class="sort-col-btn${dateActive ? ' active' : ''}" onclick="location='${buildDashboardHref(filter, 'date', searchOptions)}'">${dateLabel} ${dateActive ? '&#8595;' : '&#8593;'}</button>
...
function renderJobPagination(filter, sort, pagination, searchOptions) {
  ...
  for (const pageNum of [...visiblePages].sort((a, b) => a - b)) {
    ...
    pageLinks.push(
      pageNum === page
        ...
        : `<a class="pagination-link" href="${buildDashboardHref(filter, sort, { ...searchOptions, page: pageNum })}">${pageNum}</a>`
    );
    lastPage = pageNum;
  }

  const prevLink = page > 1
    ? `<a class="pagination-btn" href="${buildDashboardHref(filter, sort, { ...searchOptions, page: page - 1 })}">&larr; Prev</a>`
    : '<span class="pagination-btn disabled">&larr; Prev</span>';
  const nextLink = page < totalPages
    ? `<a class="pagination-btn" href="${buildDashboardHref(filter, sort, { ...searchOptions, page: page + 1 })}">Next &rarr;</a>`
...
function renderJobTable(jobs, appliedByCompany, companyTags, filter, sort, pagination, searchOptions = {}) {
  filter = filter || 'all';
  sort = sort || 'score';
  if (!jobs.length) {
    return renderJobListHeader(filter, sort, searchOptions) + '<div class="empty">No jobs found for this filter.</div>';
  }
  ...
  const cards = jobs.map(j => {
    ...
    const jobTitleHtml = renderJobTitle(j, eid, eTitle);
    ...
    if (j.apply_complexity === 'custom-url') {
      badgeParts.push('<span class="complexity-badge custom-url" title="Custom apply domain">custom url</span>');
    }
    ...
    // Start expanded when reasoning exists so it's visible without clicking
    const cardExtra = (isHot ? ' score-hot' : '') + (eReasoning ? ' expanded' : '');
    ...
    const rawDesc = (j.description || '').replace(/\s+/g, ' ').trim();
    const descPanel = rawDesc
      ? `<div class="desc-snippet">${escapeHtml(rawDesc.slice(0, 220))}${rawDesc.length > 220 ? '&hellip;' : ''} <button class="desc-more-btn" onclick="openJobDescription(${jsString(j.id)},${jsString(j.title)},${jsString(j.company)})">View full</button></div>`
      : '';
    ...
    const colDateValue = isRejected ? (j.rejected_at || j.updated_at) : (j.posted_at || j.created_at);
    ...
    <div class="job-card${cardExtra}" data-id="${eid}" data-status="${escapeHtml(j.status)}" data-posted-ts="${postedTimestamp(j.posted_at)}" data-applied-ts="${postedTimestamp(j.applied_at)}" style="--score-color:${scoreColor(sc)}">
      <div class="job-col-score">
        ${sc != null ? `<span class="score-circle ${scoreClass(sc)}">${sc}</span>` : `<span class="score-circle ${scoreClass(sc)}" title="Awaiting score" style="font-size:9px;letter-spacing:0.5px;color:#9ca3af">scoring</span>`}
      </div>
      <div class="job-col-info">
        <div class="job-title">${jobTitleHtml}</div>
      ...
      <div class="job-col-actions">
        ${(() => { const isSel = !!j.resume_requested; return `<button class="job-select-btn${isSel ? ' selected' : ''}" onclick="toggleResume('${eid}', ${isSel ? 0 : 1}, this)" title="${isSel ? 'Deselect' : 'Select'}">${isSel ? ICON_CHECK : ''}</button>`; })()}
  ...
  return renderJobListHeader(filter, sort, searchOptions)
    + `<div class="job-list filter-${escapeHtml(filter)}" id="job-tbody">${cards}</div>`
    + renderJobPagination(filter, sort, pagination, searchOptions);
}
