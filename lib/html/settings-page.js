...
function renderSettingsPage() {
  ...
  const apiKeysSection = section('api-keys', 'API Keys',
    'Writes to .env. Restart the dashboard after changes for them to take effect in background workers.',
    2,
    field('s-ollama-host',     'Ollama Host',               { placeholder: 'http://localhost:11434',       hint: 'Optional local LLM endpoint — replaces Claude CLI for scoring. From Docker use http://host.docker.internal:11434' }) +
  ...
  const profileSection = section('profile', 'Profile',
    'Writes to data/context.md. This is what the coarse filter and scorer read to understand your goals, preferences, and dealbreakers. Edit freely — it is plain Markdown.',
    1,
