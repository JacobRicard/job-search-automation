...
<div class="modal-overlay" id="onboarding-wizard">
  <div class="modal wizard-modal" style="max-width:580px;max-height:90vh;overflow-y:auto">
    ...
    <div class="wizard-step active" id="wstep-0">
      ...
      <p class="wizard-note">Setup takes about 3 minutes. Scoring uses the local Claude CLI — no separate API key required.</p>
    ...
    <div class="wizard-step" id="wstep-1">
      <h2>Claude CLI</h2>
      <p class="modal-sub">Scoring uses your local <code>claude</code> CLI — no separate API key needed. Uses your existing Claude subscription.</p>
      <div id="wizard-key-status" style="font-size:12px;color:var(--text-muted);min-height:18px;margin-bottom:14px"></div>
      <div style="display:flex;gap:8px;margin-bottom:16px">
        <button class="btn" style="background:var(--slate);color:var(--text-muted);white-space:nowrap" onclick="wizardTestKey()" id="wizard-test-btn">Verify claude CLI</button>
      </div>
      <p class="wizard-note">Install Claude Code from <a href="https://claude.ai/code" target="_blank" rel="noopener" style="color:${COLORS.accent}">claude.ai/code</a> if not already installed, then sign in.</p>
      <div class="modal-actions" style="margin-top:24px">
        <button class="btn" style="background:var(--slate);color:var(--text-muted)" onclick="wizardBack()">Back</button>
        <button class="btn" style="background:transparent;color:var(--text-muted);border:1px solid var(--border)" onclick="wizardNext()">Skip</button>
        <button class="btn" style="background:${COLORS.accent};color:white" onclick="wizardNext()" id="wizard-key-save-btn">Continue &rarr;</button>
      </div>
    </div>
