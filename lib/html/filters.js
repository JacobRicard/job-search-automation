...
function renderLocationDropdown(locationPrefs, metros) {
  ...
  const metroStyle = active ? ` style="color:var(--accent)"` : '';
  const includeUnknown = prefs.includeUnknown !== false;
  const unlistedStyle = !includeUnknown ? ` style="color:var(--accent)"` : '';

  return `<select class="metro-select"${metroStyle} onchange="applyMetroSelect(this)">
    <option value="">All locations</option>
    ${options}
  </select>
  <select class="metro-select"${unlistedStyle} onchange="applyUnlistedSelect(this)">
    <option value="show"${includeUnknown ? ' selected' : ''}>Show unlisted</option>
    <option value="hide"${!includeUnknown ? ' selected' : ''}>Hide unlisted</option>
  </select>`;
}
