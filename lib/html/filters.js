...
function renderLocationDropdown(locationPrefs, metros) {
  ...
  const fullTitle = (() => {
    ...
    all.push(prefs.remote === false ? 'Remote excluded' : 'Remote included');
  ...
  const remoteChecked = prefs.remote !== false;
  ...
  return `<div class="dd-wrap">
    ...
    <div id="location-panel" class="filter-dropdown loc-dropdown" role="menu">
      ...
      <label class="loc-option${remoteChecked ? ' checked' : ''}">
        <input type="checkbox" id="loc-remote"${remoteChecked ? ' checked' : ''} onchange="onLocationChange()">
        <span class="loc-option-check" aria-hidden="true"></span>
        <span class="loc-option-label">Include remote jobs</span>
      </label>
