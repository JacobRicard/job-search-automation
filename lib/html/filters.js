...
function renderLocationDropdown(locationPrefs, metros) {
  ...
  const metroOptions = Object.entries(metros).map(([key, m]) => {
    const selected = selectedMetros.has(key);
    return `<option value="${escapeHtml(key)}"${selected ? ' selected' : ''}>${escapeHtml(m.label || key)}</option>`;
  }).join('');
  ...
  return `<div class="dd-wrap">
    ...
    <div id="location-panel" class="filter-dropdown loc-dropdown" role="menu">
      ...
      <select multiple class="loc-select" id="loc-select" size="6">${metroOptions}</select>
      <label class="loc-unlisted-row">
        <input type="checkbox" id="loc-include-unlisted"${includeUnknown ? ' checked' : ''}>
        <span class="loc-unlisted-label">Include jobs with no listed location</span>
      </label>
