...
function renderLocationDropdown(locationPrefs, metros) {
  ...
  const fullTitle = active
    ? 'Metros: ' + [...selectedMetros].map((k) => metros[k]?.label || k).join(', ')
      + '\nRemote and unknown-location jobs are always included'
  ...
  const metroRows = Object.entries(metros).map(([key, m]) => {
    ...
    return `<label class="loc-option${checked ? ' checked' : ''}">
      <input type="checkbox" data-metro="${escapeHtml(key)}"${checked ? ' checked' : ''} onchange="this.parentElement.classList.toggle('checked', this.checked)">
