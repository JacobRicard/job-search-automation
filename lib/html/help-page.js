...
${sub(115, 178, 'refresh.js  ·  active profile', '#c4b5fd', 0.65)}
...
function renderHelpPage() {
  ...
  const architectureDiagram = [
    ...
    '    |',
    '    +--> refresh.js',
    '    |      +--> load repo .env + active profile .env',
    ...
    '    |      +--> node sync-rejection-emails.js',
    '    |      +--> node validate-slugs.js',
  ...
  const routesCode = [
    ...
    'GET  /auto-apply-attempt      apply receipt JSON',
    'GET  /auto-apply-artifact     stream receipt screenshot or resume',
    'GET  /api/tracker             apply + rejection stats',
    'GET  /api/insights            daily digest + chart data',
    'GET  /healthz                 DB health probe',
    'GET  /metrics                 Prometheus metrics',
    'GET  /public/*                static CSS, JS, and bookmarklet assets',
    '',
    '# Write',
  ...
  const fileMapCode = [
    ...
    'refresh.js                    refresh orchestration entrypoint',
  ...
  const npmCommandsCode = [
    ...
    'npm run daily              # run same active-profile flow as refresh',
...
<main class="help-page-shell">
  ...
  <section id="quick-commands" class="help-page-section">
    ...
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        eyebrow: 'npm scripts',
        title: 'Shared command catalog',
        body: 'The dashboard help page and <code>npm run help</code> use the same command catalog.',
        html: renderCommandCatalog(commandGroups),
      })}
      ${renderCard({
        eyebrow: 'Apply CLI',
  ...
  <section id="runtime" class="help-page-section">
    ...
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        ...
        bullets: [
          '<code>7 8 * * *</code> runs <code>npm run refresh</code>',
          '<code>7 14 * * *</code> runs <code>npm run refresh</code>',
          '<code>7 20 * * *</code> runs <code>npm run refresh</code>',
  ...
  <section id="pipeline" class="help-page-section">
    <div class="help-page-section-head">
      ...
      <h2>What <code>refresh.js</code> Actually Does</h2>
    </div>
    <div class="help-page-grid help-page-grid-2">
      ${renderCard({
        eyebrow: 'Active profile',
        title: 'Profile isolation is env-driven',
        body: 'The refresh runner loads repo <code>.env</code>, then loads the active profile&apos;s <code>.env</code>. Manual and scheduled refreshes now use the same Node orchestration path.',
        bullets: [
      ...
      ${renderCard({
        ...
        bullets: [
          ...
          '<code>node auto-ghost.js</code> marks stale applied jobs with no response as ghosted',
          ...
          '<code>node sync-rejection-emails.js</code>, <code>node validate-slugs.js --broken-only</code>, and <code>node update-context.js</code> run at the end',
        ],
      })}
      ${renderCard({
        eyebrow: 'Scraping',
    ...
    <div class="help-page-grid help-page-grid-2">
      ...
      ${renderCard({
        ...
        bullets: [
          ...
          '<code>auto_apply_log</code>: receipts from assisted apply attempts, including dry runs and failures',
    ...
    <div class="help-page-grid help-page-grid-2">
      ${renderCard({
        ...
        bullets: [
          ...
          'Chart.js is loaded from a CDN only when a chart view needs it',
  ...
  <section id="dashboard-views" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">Dashboard views</div>
      <h2>What Each View Is For</h2>
    </div>
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        ...
        title: 'Job list tabs',
        bullets: [
          '<code>All</code> shows active jobs that are not archived, rejected, closed, or ghosted',
          '<code>Not Applied</code> is the main triage queue for jobs that still need a decision',
          '<code>Applied</code> keeps active submitted applications visible for follow-up',
          '<code>Interviewing</code> groups phone screen, interview, onsite, and offer stages',
          '<code>Rejected</code>, <code>Closed</code>, <code>Ghosted</code>, and <code>Archived</code> are terminal or hidden-state review queues',
        ],
      })}
      ${renderCard({
        eyebrow: 'Analytics',
        title: 'Stats, receipts, and events',
        bullets: [
          '<code>Stats</code> shows all-time funnel counts, score calibration, recent events, and rejection timing',
          '<code>Apply Receipts</code> reads <code>auto_apply_log</code> and can be filtered by status, platform, mode, failure class, score, and age',
          '<code>Event Log</code> shows the audit trail written by stage, outreach, and archive actions',
          'Receipt details open through <code>GET /auto-apply-attempt</code>; screenshots and resumes stream through <code>GET /auto-apply-artifact</code>',
        ],
      })}
      ${renderCard({
        eyebrow: 'Market',
        ...
        bullets: [
          'Reads the current market research cache from the active profile',
          'Compares live and all-time job descriptions against the active resume',
          'Shows seniority distribution, location breakdown, top skills, strategy score, and high-score emerging signals',
          '<code>POST /market-research</code> regenerates the cache with Gemini and redirects back to the view',
        ],
      })}
    </div>
  </section>

  <section id="job-actions" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">Job actions</div>
      <h2>What You Can Do From A Job Card</h2>
    </div>
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        eyebrow: 'Review',
        title: 'Inspect the job',
        bullets: [
          '<code>Reasoning</code> toggles the stored score or rejection reasoning',
          '<code>View JD</code> opens the saved job description from <code>GET /job-description</code>',
          '<code>View Apply Image</code> opens pre-apply or post-apply screenshots when artifacts exist',
          'Badges summarize ATS, salary, company tags, existing applications at that company, tailored resume status, and AI-resume relevance',
        ],
      })}
      ${renderCard({
        eyebrow: 'Apply',
        ...
        bullets: [
          '<code>Manual Apply Prep</code> opens <code>GET /job-application-prep</code> in a new tab',
          '<code>Copy Answers</code> fetches <code>GET /job-application-data</code> and opens copyable generated answers',
          '<code>Tailor Resume</code> calls <code>POST /tailored-resume</code> and stores per-job Markdown, HTML, PDF, and metadata artifacts',
          '<code>View Tailored Resume</code> appears once the generated PDF is ready',
        ],
      })}
      ${renderCard({
        eyebrow: 'Tracking',
        ...
        bullets: [
          'The stage dropdown calls <code>POST /pipeline</code> and writes <code>status</code>, <code>stage</code>, and audit events',
          'Valid stages are pending, applied, phone screen, interview, onsite, offer, closed, rejected, and ghosted',
          '<code>Reach out</code> toggles <code>reached_out_at</code> through <code>POST /mark-outreach</code>',
          '<code>Archive</code> hides the job from active views through <code>POST /archive</code> without deleting it',
        ],
      })}
    </div>
  </section>

  <section id="search-filters" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">Search and filters</div>
      <h2>How List Filtering Works</h2>
    </div>
    <div class="help-page-grid help-page-grid-2">
      ${renderCard({
        eyebrow: 'URL state',
        title: 'Every list view is linkable',
        bullets: [
          '<code>filter</code> selects the current view; legacy <code>auto-apply-log</code> redirects internally to <code>auto-applies</code>',
          '<code>sort=score</code> ranks by score first; <code>sort=date</code> ranks by posted, applied, or rejected date depending on the view',
          '<code>level=1</code> enables My Level seniority filtering',
          '<code>q</code>, <code>minScore</code>, and <code>page</code> are preserved in dashboard links where relevant',
        ],
      })}
      ${renderCard({
        eyebrow: 'Search',
        title: 'What the search box matches',
        bullets: [
          'Search is case-insensitive and normalizes repeated whitespace',
          'It checks title, company, location, description, reasoning, rejection reasoning, status, stage, apply complexity, and platform',
          'The min-score slider filters the current list before pagination',
          'Paginated job views show 25 jobs per page; analytics, receipts, events, and market research are not paginated by the job-list pager',
        ],
      })}
    </div>
  </section>

  <section id="apply-workflow" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">Apply workflow</div>
      <h2>Prep, Bookmarklet, Resumes, And Receipts</h2>
    </div>
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        eyebrow: 'Prep page',
        title: 'Generated application pack',
        bullets: [
          '<code>GET /job-application-prep</code> renders questions, generated answers, voice-check status, profile data, and JSON payload',
          '<code>POST /job-application-prep</code> generates or refreshes prep for a job and returns a redirect URL',
          'Prep falls back to manual overrides when generated prep is missing',
          'The page exposes a bookmarklet string tied to the selected job id',
        ],
      })}
      ${renderCard({
        eyebrow: 'Bookmarklet',
        title: 'Browser-side autofill',
        bullets: [
          '<code>GET /job-bookmarklet.js</code> returns a job-specific script payload',
          'The script uses the same application data as the prep page',
          'Best-supported ATS platforms are Greenhouse, Lever, and Ashby',
          'The intended workflow is review-and-submit by a human, not blind final submission',
        ],
      })}
      ${renderCard({
        eyebrow: 'Artifacts',
        title: 'Generated files and receipts',
        bullets: [
          'Tailored resumes stream from <code>GET /tailored-resume</code> as PDF by default, with <code>type=html</code> or <code>type=md</code> available',
          'Apply screenshots stream from <code>GET /job-apply-image</code> and are discovered through <code>GET /job-apply-images</code>',
          'Apply receipts capture status, mode, platform, failure class, screenshots, and resume artifacts for later audit',
          'Receipt views are documentation and diagnostics for assisted workflows; they do not change job state by themselves',
        ],
      })}
    </div>
  </section>

  <section id="apis" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">APIs</div>
      <h2>Data Endpoints Used By The Dashboard</h2>
    </div>
    <div class="help-page-grid help-page-grid-2">
      ${renderCard({
        eyebrow: 'Read APIs',
        title: 'JSON and artifact endpoints',
        bullets: [
          '<code>GET /api/insights</code> returns today&apos;s activity counts, daily digest, scraper health, and daily applied chart rows',
          '<code>GET /api/tracker?period=7d|30d|90d|all</code> returns tracker rows for the market research chart',
          '<code>GET /company-notes?company=&lt;name&gt;</code> returns tags and notes for a normalized company key',
          '<code>GET /resume?variant=ai|devops</code> streams active-profile resume PDFs when present',
          '<code>GET /public/*</code> serves dashboard CSS, JS, and bookmarklet assets with cache headers based on version query strings',
        ],
      })}
      ${renderCard({
        eyebrow: 'Write APIs',
        title: 'State-changing routes',
        bullets: [
          '<code>POST /pipeline</code>, <code>/mark-outreach</code>, <code>/archive</code>, and <code>/company-notes</code> mutate SQLite and return JSON',
          '<code>POST /tailored-resume</code> and <code>/job-application-prep</code> may call Gemini and write generated artifacts',
          '<code>POST /dismiss-slug-banner</code> records the current slug-health timestamp as dismissed',
          'All dashboard mutation routes are local HTTP routes in the same Node process; there is no separate API service',
        ],
      })}
    </div>
  </section>

  <section id="diagnostics" class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">Diagnostics</div>
      <h2>Health, Metrics, And Warnings</h2>
    </div>
    <div class="help-page-grid help-page-grid-3">
      ${renderCard({
        eyebrow: 'Health',
        ...
        bullets: [
          '<code>GET /healthz</code> returns <code>{ ok: true }</code> when SQLite responds to <code>SELECT 1</code>',
          '<code>GET /metrics</code> exposes Prometheus text metrics for requests, durations, jobs by status, jobs by stage, and scraped jobs by platform',
          'Gauge metrics refresh every 60 seconds and also record status snapshots',
        ],
      })}
      ${renderCard({
        eyebrow: 'Warnings',
        title: 'Slug and JD health',
        bullets: [
          '<code>slug-health.json</code> can trigger a broken-slug banner in the dashboard',
          'Dismissing the slug banner writes <code>slug-health-dismissed.json</code> for the current timestamp',
          '<code>jd-health.json</code> is loaded into the dashboard context for job-description quality warnings',
        ],
      })}
      ${renderCard({
        eyebrow: 'Background sync',
        title: 'Rejection email poller',
        bullets: [
          'The dashboard starts the rejection-email poller in-process after the HTTP server boots',
          'It runs only when Gmail credentials are configured',
          'Matched rejection emails update the corresponding job and write audit rows',
          'Use <code>npm run sync-rejections -- --dry-run</code> for one-shot debugging without waiting for the poller',
        ],
      })}
    </div>
  </section>
