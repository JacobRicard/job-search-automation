...
const { DASHBOARD_PORT, DAILY_TARGET, SCORE_STRONG, SCORE_GOOD, SCORE_MID, SCORE_BORDERLINE, GHOSTED_AFTER_DAYS } = require('../../config/constants');
...
function renderHelpPage() {
  const setupCode = [
    'cp .env.example .env',
    '# Scoring uses Claude CLI — make sure `claude` is installed and authenticated',
    'docker compose up -d',
  ...
  const tableCode = [
    ...
    'api_usage             LLM daily usage counters (Claude + Gemini)',
  ...
  const pathRows = [
    ...
    ['LLM', 'Claude CLI (uses your subscription)'],
  ...
  const setupSection = renderSection('setup', 'Setup', 'Run It Locally', 2, [
    ...
    renderCard({
      eyebrow: 'Scoring LLM',
      title: 'Powered by Claude CLI',
      body: `Jobs are scored using your local ${pill('claude')} CLI — no API key or separate account needed. Scoring uses your existing Claude subscription.`,
      bullets: [
        `Install Claude Code from ${pill('claude.ai/code')} and sign in`,
        `Set ${pill('CLAUDE_SCORE_MODEL')} to choose the model (default: claude-sonnet-4-6)`,
        `Set ${pill('CLAUDE_FILTER_MODEL')} for the fast coarse-filter step (default: claude-haiku-4-5-20251001)`,
