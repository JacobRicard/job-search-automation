...
const DEFAULT_TRASH_UID_OVERLAP = 200;
...
async function sweepMailbox(db, mailbox, metadataPrefix, options) {
  ...
  const result = await fetchMessages({
    ...
    uidOverlap: options.uidOverlap || 0,
  });
...
async function syncRejectionEmails(db, options = {}) {
  ...
  const trash = options.skipTrash
    ...
    : await sweepMailbox(db, TRASH_MAILBOX, 'rejection_email_trash', {
      ...options,
      uidOverlap: options.trashUidOverlap ?? DEFAULT_TRASH_UID_OVERLAP,
    });
...
function startRejectionEmailPoller(db, options = {}) {
  ...
  const hasCredentials = (process.env.IMAP_EMAIL || process.env.GMAIL_EMAIL) &&
    (process.env.IMAP_APP_PASSWORD || process.env.GMAIL_APP_PASSWORD);
  if (!hasCredentials) {
    log.info('Rejection email sync not started, missing IMAP credentials');
    return { stop() {} };
  }
