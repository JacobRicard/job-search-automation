...
describe('dashboard filter links', () => {
  it('builds dashboard URLs with persisted search state', () => {
    assert.equal(
      buildDashboardHref('all', 'score', { q: 'platform aws', minScore: 7, page: 3 }),
      '/?filter=all&sort=score&q=platform+aws&minScore=7&page=3'
    );
  });
  ...
  it('renders filter controls with current search values', () => {
    ...
    assert.match(html, /href="\/\?filter=rejected&sort=date&q=Acme\+remote&minScore=6"/);
  });
});
